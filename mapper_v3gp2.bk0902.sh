...
ffmpeg -y -i $outpath/$filename.mp4 -vcodec libx264 -pix_fmt yuv420p  -vprofile baseline -preset slow -b:v 170k -maxrate 170k -bufsize 340k -vf "movie=/usr/hadoop/newlogo.png,scale=50:-1[watermark];movie=/usr/hadoop/pindaologo50px.png,scale=20:-1[watermark2];[in]${autocrop}scale=320:trunc(ow/a/2)*2 [scale];[scale][watermark]overlay=10:10[1];[1][watermark2]overlay=main_w-overlay_w-10:main_h-overlay_h-10[out]" -acodec libfdk_aac -b:a 16k -ac 2 -af 'volume=1.5' $outpath/$type/$filename.3gp < a 2>&1
