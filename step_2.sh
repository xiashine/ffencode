...
if [ $? -ne 0 ]; then
		...
		encodetit=`echo ${encodeinfo//,/ } | awk '{ print $4 }'`
		if [ "${encodetit}" != "0" ] && [ -f $splitpath$filename.00001.mp4 ]; then
			/data/encode/ffmpeg_build/bin/ffmpeg  -i $splitpath$filename.00001.mp4 -vcodec libx264 -vprofile high -preset slow -b:v 3000k -vf drawtext="fontfile=/data/encode/ffmpeg/simhei.ttf: text='互联网视听节目备案编号：${encodetit}':x=(w-text_w)/2:y=(h-text_h):enable='if(gt(n,0),lt(n,250))':fontsize=48:fontcolor=white" -acodec aac -b:a 256k -strict -2  $splitpath$filename.00001.text.mp4
			if [ -f $splitpath$filename.00001.text.mp4 ]; then
				rm -rf $splitpath$filename.00001.mp4
				mv $splitpath$filename.00001.text.mp4 $splitpath$filename.00001.mp4
			fi
		fi
		...
		autocrop=`/usr/local/bin/ffmpeg -ss 60 -i $filepath -t 300 -vf select='eq(pict_type\,PICT_TYPE_I)',cropdetect -f null - 2>&1 | awk '/crop/ { print $NF }' |sort|uniq -c|sort -nr|head -1 | awk '{print $2 }'`
		...
		/usr/local/bin/ffmpeg -y -i $filepath -vf "select='not(mod(n\,1500))',scale=128:72,tile=10x10" -an -vsync 0  ${imagepath}/${filename}.%03d.png
		if [ -f "${imagepath}/${filename}.001.png" ]; then
			pngfilepath="${imagepath}/${filename}.*.png"
			size=`ls -l ${imagepath}/${filename}.001.png | awk '{ print int($5) }'`
			timelen=`ls -l $pngfilepath | wc -l`
			curl -s -d "vid=${filename}&timelen=${timelen}&size=${size}&videopath=/images/${dt}/${filename}.TP_NUM.png&status=27&channelid=27" "$DOMAIN/api.php?op=video_api&action=edit_video_status"
		fi
fi
