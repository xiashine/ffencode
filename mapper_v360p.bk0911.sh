...
ffmpeg -y -i $outpath/$filename.mp4 -vcodec libx264 -pix_fmt yuv420p -vprofile baseline -preset slow -b:v 300k -maxrate 300k -bufsize 600k -vf "${encodeinfo}" -acodec libfdk_aac -ac 2 -b:a 32k -af 'volume=1.5' $outpath/$type/$filename.qt.mp4 < a 2>&1
