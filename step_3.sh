...
key1905="1905"
linkmp4file="${type}#${filename}@${key1905}"
linkflvfile="${type}flv#${filename}@${key1905}"
md5mp4filename=`echo -n ${linkmp4file} |md5sum |cut -d ' ' -f1 |  tr '[a-z]' '[A-Z]' |cut -c1-25`
md5flvfilename=`echo -n ${linkflvfile} |md5sum |cut -d ' ' -f1 |  tr '[a-z]' '[A-Z]' |cut -c1-25`
...
if [ "$prex" == "mp4" ]; then
	...
	/usr/bin/qt-faststart  $dir/qt_$filename.$prex  $dir/$md5mp4filename.$prex
else
	/usr/local/bin/mkvmerge -o $dir/tmp_$filename.$prex `cat $dir/files.txt`
	/usr/local/bin/ffmpeg -y -i $dir/tmp_$filename.$prex -vcodec copy -acodec copy $dir/$md5mp4filename.$prex
fi
if [ "$type" == "v720p" ]||[ "$type" == "v360p" ]||[ "$type" == "v480p" ]; then
	...
	/usr/local/bin/yamdi -i $dir/tmp_$filename.flv -o $dir/$md5flvfilename.flv
	mv $dir/$md5flvfilename.flv $flvbasepath
	$BASE/encode/m3u8_encode.sh $dir/ $md5mp4filename $m3u8basepath
fi
#if [ "$type" == "v720p" ]&&[ $channelid -eq 30 ]; then
...
#/usr/hadoop/bin/hadoop fs -put $dir/$md5mp4filename.$prex  $hadooppath
mv $dir/$filename.00001.$prex $basepath
mv $dir/$md5mp4filename*.$prex $basepath
rm -rf $dir
echo  "$(date +%Y-%m-%d) $(date +%T) 	${filename} $type	step3:reduce end"

echo "$(date +%Y-%m-%d) $(date +%T)    ${filename} $type       step4:update vms"
#step 4 update vms
if [ -f "$basepath/$md5mp4filename.$prex" ]; then
	status=0
	status_flv=0
	status_m=0
	status_c=0
	...
	size=`ls -l $basepath/$md5mp4filename.$prex | awk '{ print int($5) }'`
	timelen=`/usr/local/bin/mediainfo "--Inform=Video;%Duration%" $basepath/$md5mp4filename.$prex`
	echo "$(date +%Y-%m-%d) $(date +%T) 	${filename} $type	step3: curl -d \"vid=${filename}&timelen=${timelen}&size=${size}&videopath=/$dt/$filename/$md5mp4filename.$prex&status=${status}&channelid=${status}\" $DOMAIN/api.php?op=video_api&action=edit_video_status"
	curl -s -d "vid=${filename}&timelen=${timelen}&size=${size}&videopath=/$dt/$filename/$md5mp4filename.$prex&status=${status}&channelid=${status}" "$DOMAIN/api.php?op=video_api&action=edit_video_status"
	
	if [ "$type" == "v1080p" ]||[ "$type" == "v720p" ]||[ "$type" == "v360p" ]||[ "$type" == "v480p" ]; then
		
		if [ -f "$flvbasepath/$md5flvfilename.flv" ]; then
			size=`ls -l $flvbasepath/$md5flvfilename.flv | awk '{ print int($5) }'`
			timelen=`/usr/local/bin/mediainfo "--Inform=Video;%Duration%" $flvbasepath/$md5flvfilename.flv`
			curl -s -d "vid=${filename}&timelen=${timelen}&size=${size}&videopath=/$dt/$filename/$md5flvfilename.flv&status=${status_flv}&channelid=${status_flv}" "$DOMAIN/api.php?op=video_api&action=edit_video_status"
		fi
		if [ -f "$m3u8basepath/$md5mp4filename.m3u8" ]; then
			curl -s -d "vid=${filename}&timelen=${timelen}&size=${size}&videopath=/$dt/$filename/$md5mp4filename.m3u8&status=${status_m}&channelid=${status_m}" "$DOMAIN/api.php?op=video_api&action=edit_video_status"
		fi
		jsoncutlens="${timelen}"
