    ...
    with open(filename) as pl:
        for line in pl:
            ...
            if line.startswith("#EXTINF:"):
                duration = int(float(line[line.rfind(':')+1:line.rfind(',')]))
                next_line_is_media = True

def process_sub_playlist(filename, output_dir, content_id, 
    ...
    with open(filename) as reading_pl:
        with open(path.join(output_dir, path.basename(filename)), "w") as writing_pl:
            for line in reading_pl:
                if len(line) == 0:
                    continue
                if line.startswith('#EXT-X-STREAM-INF:') or line.startswith('#EXT-X-TARGETDURATION:') :
                    if key_tag:
        ...
        with open(path.join(output_dir, path.basename(filename)), "w") as writing_pl:
            for line in reading_pl:
                ...
                if next_line_is_playlist:
                    ...
                    yield variant
                    next_line_is_playlist = False
                elif line.startswith('#EXT-X-STREAM-INF:') or  line.startswith('#EXT-X-TARGETDURATION:') :
                    if key_tag:
                        writing_pl.write(key_tag)
                        key_tag = None # write it only once
...
def main(args):
    ...
    option_list = [
        make_option('-k', '--key', default=None,
                    type='string', help="if specified must be 32 hex chars (representing 16 bytes),"\
    ...
    for variant_playlist in process_main_playlist(args[1], 
        ...
        process_sub_playlist(path.join(input_dir, variant_playlist),
                                                path.dirname(out_variant_playlist),
                                                args[0], 
                                                opts.silent_rights_url,
                                                opts.rights_issuer_url,
                                                opts.preview_rights_url)
