...
ffmpeg -y -i $outpath/$filename.mp4 -vcodec libx264 -vprofile high -preset slow -b:v 500k -maxrate 500k -bufsize 1000k -vf "movie=/usr/hadoop/newlogo.png,scale=140:-1[watermark];movie=/usr/hadoop/pindaologo.png,scale=40:-1[watermark2];[in]${autocrop}scale=720:trunc(ow/a/2)*2 [scale];[scale][watermark]overlay=10:10[1];[1][watermark2]overlay=main_w-overlay_w-10:main_h-overlay_h-10[out]" -acodec libfdk_aac -b:a 64k -ac 2 -af 'volume=1.5' $outpath/$type/$filename.qt.mp4 < a 2>&1
